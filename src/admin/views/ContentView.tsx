...
const quickKeys = ['footer.cta_text']
...
export default function ContentView({ records, busy, onSave }: { records: CmsContentRecord[]; busy: boolean; onSave: (record: Draft) => Promise<void> }) {
  ...
  const quickRecords = useMemo(() => quickKeys.map((key) => filtered.find((record) => record.content_key === key)).filter((record): record is CmsContentRecord => !!record), [filtered])
  const grouped = useMemo(() => { const map = new Map<string, CmsContentRecord[]>(); filtered.filter((record) => !quickKeys.includes(record.content_key)).forEach((record) => { const group = groupFor(record); map.set(group, [...(map.get(group) || []), record]) }); return [...map.entries()] }, [filtered])
  ...
  const renderRecord = (record: CmsContentRecord) => <article className={styles.recordRow} key={record.content_id}><div className={styles.recordMain}><div className={styles.recordTitleLine}><strong>{record.label || record.content_key}</strong><StatusBadge status={record.status} /></div><code>{record.content_key}</code><p>{record.value || 'Sin contenido'}</p></div><div className={styles.recordActions}>{record.status === 'archived' ? <button className={styles.iconTextButton} type="button" onClick={() => void changeStatus(record,'draft')} disabled={busy}><RotateCcw size={15} /> Restaurar</button> : <><button className={styles.iconTextButton} type="button" onClick={() => openEdit(record)}><Edit3 size={15} /> Editar</button><button className={styles.iconTextButtonDanger} type="button" onClick={() => setConfirming(record)}><Archive size={15} /> Archivar</button></>}</div></article>

  return (
    <section className={styles.pageSection}>
      <div className={styles.sectionHeading}><div><span className={styles.eyebrow}>Copy administrable</span><h2>Textos y cierres de la landing</h2><p>Acá podés cambiar portada, secciones, botones y el cierre del footer. Dejamos arriba los campos que suelen costar más encontrar.</p></div><button className={styles.primaryButton} type="button" onClick={openCreate}><Plus size={17} /> Nuevo texto</button></div>
      ...
      {!!quickRecords.length && <section className={styles.contentGroup}><div className={styles.contentGroupHeader}><h3>Accesos rápidos</h3><span>Cierre de la web</span></div><div className={styles.recordTable}>{quickRecords.map(renderRecord)}</div></section>}

      {grouped.length ? <div className={styles.contentGroups}>{grouped.map(([group,items]) => <section className={styles.contentGroup} key={group}><div className={styles.contentGroupHeader}><h3>{group}</h3><span>{items.length} campo{items.length === 1 ? '' : 's'}</span></div><div className={styles.recordTable}>{items.map(renderRecord)}</div></section>)}</div> : !quickRecords.length && <EmptyState title="No encontramos textos" description="Probá cambiando los filtros o creá un nuevo campo administrable." action={<button className={styles.secondaryButton} type="button" onClick={openCreate}><Plus size={16} /> Crear texto</button>} />}
